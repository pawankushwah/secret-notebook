...
export function Dashboard() {
    ...
    async function getUserData() {
        let response = await fetch("http://127.0.0.1:3333/getUserData", {
            method: "POST",
            headers: {
    ...
    async function logoutTheUser() {
        try {
            setIsLoading(true);
            let response = await fetch("http://127.0.0.1:3333/logout", {
                method: "DELETE",
                headers: {
                    'Content-Type': "application/json"
                },
    ...
    async function uploadSecretNote() {
        let response = await fetch("http://127.0.0.1:3333/setSecret", {
